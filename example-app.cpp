...
int web_data_size = 1023452;

class information_Node* information_table[1023452] = {nullptr};
            ...
            if(hash_table[hash_index] == NULL){
                ...
                while(trigger == 0){
                    ...
                    if(search_hash == hash_string){
                        ...
                        int compressedSize = LZ4_compress_default(Self, output, 4096, LZ4_compressBound(4096));
                        ...
                        char* output_XOR = (char*)malloc(LZ4_compressBound(4096));

                        int compressedSize_XOR = LZ4_compress_default(XOR, output_XOR, 4096, LZ4_compressBound(4096));

                        if(compressedSize_XOR < compressedSize){
                            unsigned int locate = ftell(output_file);

                            fwrite(output_XOR, 1, compressedSize, output_file);
                            class information_Node* inode = new information_Node;
                            ...
                            inode->initialize_information((unsigned int)compressedSize_XOR, locate, referr, 0, 0);
